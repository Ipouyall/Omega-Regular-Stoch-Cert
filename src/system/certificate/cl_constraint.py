...
class ControllerBounds(Constraint):
    ...
    def _extract_boundaries_for_transition(self, transition: Equation, minimum: Equation, maximum: Equation):
        _ineqs = []
    ...
    def extract(self) -> list[ConstraintInequality]:
        ...
        for policy in self.decomposed_control_policy.policies:
            for tr in policy.transitions:
                _ineqs = self._extract_boundaries_for_transition(tr, min_limit, max_limit)
                if not _ineqs:
                    continue
                constraints.append(
